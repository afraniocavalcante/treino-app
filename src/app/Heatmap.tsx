...
export default function Heatmap({ trainedDates }: { trainedDates: Set<string> }) {
    ...
    <div style={{ background: C.bgCard, border: `1px solid ${C.bgHeader}`, borderRadius: 16, padding: "16px 16px 14px" }}>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 12 }}>
        ...
        <span style={{ fontSize: 11, color: C.midGray }}>
          {active ? `${formatDateDisplay(active.dateStr)} — ${active.trained ? "treinou" : "sem treino"}` : "últimas 20 semanas"}
        </span>
      </div>
